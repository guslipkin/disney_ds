...
```{r}
rides_df %>% ggplot(aes(x = SPOSTMIN, y = ride_name)) +
...
```{r}
rides_df %>% ggplot(aes(x = SACTMIN, y = ride_name)) +
  geom_boxplot(na.rm = TRUE, outlier.shape = "circle", outlier.alpha = .1, size = 1) +
...
for (i in 1:14) {
  chart <- rides[[i]] %>%
    ...
    ggplot(aes(x = ordered(weekday, levels = c("Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday")), y = mean_wait)) +
    geom_col() +
...
rides_df %>%
  ...
  summarise(mean_wait = mean(SPOSTMIN, na.rm = TRUE)) %>%
  ggplot(aes(x = ordered(weekday, levels = c("Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday")), 
             y = mean_wait, fill = ride_name), color = as.factor(ride_name)) +
  geom_col()
```
